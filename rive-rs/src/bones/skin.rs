...
impl ObjectRef<'_, Skin> {
    ...
    pub fn deform(&self, vertices: impl Iterator<Item = Object<PathVertex>>) {
        let bone_transforms = self
            .bone_transforms
            .get()
            .expect("bone_transforms should already be set");
        for vertex in vertices {
            vertex
                .as_ref()
                .deform(self.world_transform.get(), bone_transforms);
        }
    }
    ...
    pub fn build_dependencies(&self) {
        for tendon in self.tendons.iter() {
            ...
            bone.as_ref()
                .cast::<Component>()
                .push_dependent(self.cast::<Component>().as_object());
        }

        self.bone_transforms
            .set(
                iter::repeat(Cell::new(Mat::default()))
                    .take(self.tendons.len() + 1)
                    .collect(),
            )
    ...
    pub fn on_dirty(&self, _dirt: FlagSet<ComponentDirt>) {
        let skinnable = self
            .skinnable
            .get()
            .expect("Skinnable should already be set");
        skinnable::as_ref(skinnable.as_ref()).mark_skin_dirty();
    }

    pub fn update(&self, _value: FlagSet<ComponentDirt>) {
        if let Some(bone_transforms) = self.bone_transforms.get() {
            for (bone_transform, tendon) in bone_transforms.iter().skip(1).zip(self.tendons.iter())
            {
                ...
                let bone = tendon
                    .bone()
                    .expect("Tendon's bone must be set")
...
impl OnAdded for ObjectRef<'_, Skin> {
    ...
    fn on_added_clean(&self, _context: &dyn CoreContext) -> StatusCode {
        ...
        self.skinnable.set(
            self.cast::<Component>()
                .parent()
                .and_then(|parent| skinnable::try_from(parent.into())),
        );
