...
fn read_runtime_object(
    ...
    let (core, object) = Core::from_type_id(id)?;
...
impl File {
    pub fn import(reader: &mut BinaryReader) -> Result<Self, ImportError> {
    ...
    fn read(reader: &mut BinaryReader, header: &RuntimeHeader) -> Option<Self> {
