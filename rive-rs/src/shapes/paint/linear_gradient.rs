...
impl ObjectRef<'_, LinearGradient> {
    ...
    pub fn build_dependencies(&self) {
            ...
            parent.and_then(|parent| parent.cast::<Component>().as_ref().parent())
        {
            ...
            self.shape_paint_container
                .set(parents_parent.try_cast::<Node>());
            parents_parent
                .cast::<Component>()
                .as_ref()
                .push_dependent(component.as_object());
        }
    }
    ...
    pub fn update(&self, value: FlagSet<ComponentDirt>) {
        ...
        if Component::value_has_dirt(value, ComponentDirt::Stops) {
            self.stops.sort_by(|a, b| {
                a.as_ref()
                    .position()
                    .partial_cmp(&b.as_ref().position())
                    .unwrap()
            });
        }

        let world_transformed = Component::value_has_dirt(value, ComponentDirt::WorldTransform);
        ...
        for stop in self.stops.iter() {
            builder.push_stop(
                stop.as_ref().color().mul_opacity(ro),
                stop.as_ref().position(),
            );
        }

        self.cast::<ShapePaintMutator>()
            .set_gradient(builder.build());
    }
}
...
impl OnAdded for ObjectRef<'_, LinearGradient> {
    ...
    fn on_added_dirty(&self, context: &dyn CoreContext) -> StatusCode {
        ...
        if let Some(parent) = self
            .cast::<Component>()
            .parent()
            .and_then(|parent| parent.try_cast::<Component>())
        {
