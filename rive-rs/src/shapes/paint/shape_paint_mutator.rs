...
impl ObjectRef<'_, ShapePaintMutator> {
    ...
    pub fn init_paint_mutator(&self, component: Object<Component>) -> bool {
        if let Some(fill) = component.try_cast::<Fill>() {
            self.render_paint
                .set(fill.as_ref().init_render_paint(self.as_object()));
            return true;
        }

        if let Some(stroke) = component.try_cast::<Stroke>() {
            self.render_paint
                .set(stroke.as_ref().init_render_paint(self.as_object()));
            return true;
        }

        if let Some(shape_paint) = component.try_cast::<ShapePaint>() {
            self.render_paint
                .set(shape_paint.as_ref().init_render_paint(self.as_object()));
            return true;
        }
