...
impl ObjectRef<'_, TransformComponent> {
    ...
    pub fn set_opacity(&self, opacity: f32) {
        ...
        self.cast::<Component>()
            .add_dirt(ComponentDirt::RenderOpacity, true);
    }
}
...
impl ObjectRef<'_, TransformComponent> {
    ...
    pub fn mark_world_transform_dirty(&self) {
        self.cast::<Component>()
            .add_dirt(ComponentDirt::WorldTransform, true);
    }
    ...
    pub fn update_world_transform(&self) {
        if let Some(parent_transform_component) = self.parent_transform_component.get() {
            ...
            self.world_transform
                .set(parent_transform * self.transform.get());
        } else {
    ...
    pub fn update(&self, value: FlagSet<ComponentDirt>) {
        ...
        if Component::value_has_dirt(value, ComponentDirt::WorldTransform) {
...
impl OnAdded for ObjectRef<'_, TransformComponent> {
    ...
    fn on_added_clean(&self, _context: &dyn CoreContext) -> StatusCode {
        let parent = self
            .cast::<Component>()
            .parent()
            .and_then(|parent| parent.try_cast());
