...
fn lli4(p0: math::Vec, p1: math::Vec, p2: math::Vec, p3: math::Vec) -> Option<math::Vec> {
    let nx =
...
fn scale(points: &[math::Vec; 4], dist: f32) -> Option<[math::Vec; 4]> {
    ...
    let offset = [points[0] + normals[0] * dist, points[3] + normals[1] * dist];
    let origin = lli4(offset[0], points[0], offset[1], points[3])?;
    ...
    new_points[1] = lli4(
        new_points[0],
        new_points[0] + derive_left(points),
        origin,
        points[1],
    )?;
    new_points[2] = lli4(
        new_points[3],
        new_points[3] + derive_right(points),
        origin,
        points[2],
    )?;
...
impl Bezier {
    ...
    pub fn intersect(&mut self, other: &mut Self) -> bool {
        let intersection = line_segment_intersection(self.as_segment(), other.as_segment());

        match intersection {
            None => false,
            Some(intersection) => {
                *self.points_mut().last_mut().unwrap() = intersection;
                *other.points_mut().first_mut().unwrap() = intersection;
            ...
            .map(|bezier| {
                if bezier.is_linear() {
                    ...
                    if let Self::Cubic(points) = bezier {
                        Self::Cubic(
                            scale(&points, dist)
                                .expect(&format!("cannot scale {:#?} by {}", points, dist)),
                        )
                    } else {
                        unreachable!()
                    }
...
mod tests {
    ...
    fn reduce() {
        let cubic = Bezier::Cubic([
            math::Vec::new(29.931055, -49.431267),
            math::Vec::new(30.053669, -24.057346),
            math::Vec::new(3.2716448, 0.3535397),
            math::Vec::new(3.2716448, 0.3535397),
        ]);

        panic!("{:#?}", cubic.offset(2.0),);
    }
}
