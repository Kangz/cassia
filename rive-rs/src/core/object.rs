...
impl<T> Object<T> {
    ...
    fn upgrade_rc(&self) -> Rc<dyn Core> {
        self.weak
            .upgrade()
            .expect("File dropped while objects still in use")
    }

    pub fn as_ref(&self) -> ObjectRef<T> {
        ObjectRef {
...
impl<'a, T: Core> CoreRef<'a, T> {
    pub fn try_cast<'b, C: Core>(&'b self) -> Option<CoreRef<'a, C>> {
        match self {
            Self::Rc(rc) => rc.is::<C>().then(|| CoreRef::Rc(Rc::clone(&rc))),
            Self::Borrow(borrow) => Core::try_cast::<C>(*borrow).map(CoreRef::Borrow),
        }
    }
...
impl<'a> ObjectRef<'a> {
    ...
    pub fn cast<'b, C: Core>(&'b self) -> ObjectRef<'a, C> {
        self.try_cast().expect(&format!(
            "failed cast from ObjectRef<'_> to {}",
            any_type_name::<C>(),
        ))
    }
