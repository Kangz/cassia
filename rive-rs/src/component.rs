...
use crate::{
    ...
    shapes::{
        paint::LinearGradient, ClippingShape, Ellipse, Path, PathComposer, PointsPath, Shape,
    },
...
pub struct Component {
    ...
    pub(crate) dirt: Cell<FlagSet<ComponentDirt>>,
}
...
impl Component {
    ...
    pub fn value_has_dirt(
        value: impl Into<FlagSet<ComponentDirt>>,
        flags: impl Into<FlagSet<ComponentDirt>>,
    ) -> bool {
        let value = value.into();
        let flags = flags.into();

        value & flags != FlagSet::default()
    }
}
...
impl ObjectRef<'_, Component> {
    ...
    pub fn parents(&self) -> impl Iterator<Item = Object<ContainerComponent>> {
        iter::successors(self.parent(), |component| {
            component.cast::<Component>().as_ref().parent()
        })
    }
    ...
    pub fn has_dirt(&self, flags: impl Into<FlagSet<ComponentDirt>>) -> bool {
        let flags = flags.into();
        self.dirt.get() & flags == flags
    }

    pub fn add_dirt(&self, value: impl Into<FlagSet<ComponentDirt>>, recurse: bool) -> bool {
        let value = value.into();
...
impl OnAdded for ObjectRef<'_, Component> {
    ...
    fn on_added_dirty(&self, context: &dyn CoreContext) -> StatusCode {
        ...
        if let Some(parent) = context
            .resolve(self.parent_id() as usize)
