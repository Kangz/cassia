...
fn populate_indices(
    ...
    let mut splits = chunks(
        lengths,
        line_indices,
        pixel_indices,
        INDICES_MAX_CHUNK_SIZE as usize,
    );
...
fn segment(
    ...
    CompactSegment::new(0, tj, ti.max(-1) + 1, layer, ty, tx, area, cover)
}
            ...
            .map(|(&li, &pi)| {
                ...
                if intersects_y_border(y0_sub, y1_sub) {
                    ...
                    let segment0 = segment(
                        layer, x0_sub, x_sub, y0_sub, y_sub, border_x, border_y, octant,
                    );

                    let border_y = y_sub.min(y1_sub) >> PIXEL_SHIFT as i32;
                    let segment1 = segment(
                        layer, x_sub, x1_sub, y_sub, y1_sub, border_x, border_y, octant,
                    );
                    ...
                    let segment0 = segment(
                        layer, x0_sub, x1_sub, y0_sub, y1_sub, border_x, border_y, octant,
                    );
