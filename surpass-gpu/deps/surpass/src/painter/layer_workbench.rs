...
impl LayerWorkbench {
    ...
    fn cover_carry<'c, P: LayerProps>(
        ...
        (!acc_cover.is_empty(context.props.get(id).fill_rule)).then(|| CoverCarry {
            cover: acc_cover,
            layer: id,
        })
    }
    ...
    fn skip_trivial_clips_pass<'c, P: LayerProps>(
        ...
        for (i, &id) in self.ids.iter_masked() {
            ...
            if let Func::Clip(layers) = props.func {
                ...
                clip = Some(Clip {
                    is_full,
                    last_layer_id: id + layers as u16,
                    i,
                    is_used: false,
                });
    ...
    fn populate_layers<'c, P: LayerProps>(&mut self, context: &'c Context<'_, P>) {
        ...
        self.queue_indices.extend(
            self.queue
                .iter()
                .enumerate()
                .map(|(i, cover_carry)| (cover_carry.layer, i)),
        );
        ...
        self.ids.extend(
            self.segment_ranges
                .keys()
                .copied()
                .chain(self.queue_indices.keys().copied()),
        );
