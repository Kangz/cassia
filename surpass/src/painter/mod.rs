...
impl LayerPainter for Painter {
    fn clear_cells(&mut self) {
        self.areas
            .iter_mut()
            .for_each(|area| *area = i16x16::splat(0));
        self.covers
            .iter_mut()
    ...
    fn clear(&mut self, color: [f32; 4]) {
        self.c0
            .iter_mut()
            .for_each(|c0| *c0 = f32x8::splat(color[0]));
        self.c1
            .iter_mut()
            .for_each(|c1| *c1 = f32x8::splat(color[1]));
        self.c2
            .iter_mut()
            .for_each(|c2| *c2 = f32x8::splat(color[2]));
        self.alpha
            .iter_mut()
            .for_each(|alpha| *alpha = f32x8::splat(color[3]));
    }
...
impl Painter {
    ...
    pub fn paint_tile_row<'c, P: LayerProps>(
        ...
        workbench.init(
            covers_left_of_row
                .into_iter()
                .map(|(layer, cover)| CoverCarry { cover, layer }),
        );
        ...
        for (i, tile) in row.enumerate() {
            ...
            let context = Context {
                ...
                previous_layers: Cell::new(
                    previous_layers
                        .as_mut()
                        .map(|layers_per_tile| &mut layers_per_tile[i]),
                ),
                clear_color,
            };
